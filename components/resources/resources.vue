<template>
  <!-- 资源 -->
  <view>
  <view class="star-card" v-for="(item, index) in resourcesList"
                :key="index">
    <view class="list-top">
      <view class="row-text">
        资源 {{item.endTime}}
    ...
    <view class="slider-area">
      ...
      <view class="btn-slider btn-slider1">{{ item.reachNumValue+'%' }}</view>
    </view>
    ...
    <view class="list-bottom">
      <view class="star-ranking">
        <view v-for="(item, index) in item.fensList" :key="index" class="img-list">
          <u-image
            class="img"
            width="60rpx"
            height="60rpx"
      ...
      resourcesList: [
         {
          ...
          "fensList": [
            {
              "avatarUrl": "https://ss1.bdstatic.com/70cFuXSh_Q1YnxGkpoWK1HF6hhy/it/u=1743749179,750499312&fm=26&gp=0.jpg",
              "fensId": 0,
              "flag": true,
              "nickName": "string",
              "vigourVal": 0
      ...
      this.$u
        ...
        .catch(res => {
          this.resourcesList.forEach((item,index)=>{
            //  处理参与人数比
            this.resourcesList[index].targetValue = item.joinNum/item.target*100
            //  处理达成人数比
            this.resourcesList[index].reachNumValue = item.reachNum/item.target*100 
          })
        });
    },
